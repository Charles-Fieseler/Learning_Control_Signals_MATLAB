...
%% Compare reconstructions with several different DMD backends
...
% Now look at the singular value decay
% plotSVD(dat_high);
...
[ ~, ~, ~, romA_naive ] = dmd( dat_high, 1, -1);
[ ~, ~, U, romA_thresh ] = dmd( dat_high, 1, 0);
Uhat = U(:, 1:length(romA_thresh));
A_thresh = Uhat*romA_thresh*Uhat';

% Next do an exponential DMD model
t = (1:size(dat,2))';
dt = t(2) - t(1);

[U,S,V] = svd(dat_high(:,1:end-1));
r = n;
Ur = U(:,1:r);
Sr = S(1:r,1:r);
Vr = V(:,1:r);
dat_high2 = Ur*Sr*Vr';
% [A_exp] = exponential_dmd(dat_high, 10, false);
% [A_exp] = exponential_dmd(dat_high, 10, false, 1, false, 0.1);
% [A_exp] = exponential_dmd(dat_high, 200, true, 0.94);
[A_exp] = exponential_dmd(dat_high2, 300, true, 0.98);
% Also get the exponential fit (not just the matrix multiplication)
[w_exp, lambda_exp] = eig(Ur'*A_exp*Ur, 'vector');
e_exp = log(lambda_exp)/dt;
Phi_exp = dat_high(:,2:end) * Vr / Sr * w_exp;
b_exp = Phi_exp \ dat_high(:,1);
dat_exp = Phi_exp*diag(b_exp)*exp(e_exp*t');

% Also do optimized DMD
[w,e,b, ~, u, A_continuous] = optdmd(dat_high, t, r, 2);
dat_opt = w*diag(b)*exp(e*t');

% Let me just try to directly undo the dat_exp transforms above
% full_eigenvec = dat_high(:,1:end-1) / dat_high(:,2:end) * w;
% A_opt = full_eigenvec*diag(exp(dt*e))/full_eigenvec;

A_opt = w*diag(exp(dt*e))/w;
% [V, D] = eig(w2*diag(e)/w2, 'vector'); % Note: w are NOT the eigenvectors!
% [V, D] = eig(A_continuous, 'vector'); % Note: w are NOT the eigenvectors!
% % V = V(:,end-r+1:end);
% % D = D(end-r+1:end);
% V = V(:,1:r);
% D = D(1:r);
% 
% A_opt = V*diag(exp(dt*D))/V;

% The eigenvalues are disordered relative to the real space; show that
% A_opt and A_opt2 have the same eigenvectors
% [V1, D1] = eig(A_opt, 'vector');
% [V2, D2] = eig(A_opt2, 'vector');
% [~, I1] = sort(D1(end-r+1:end));
% [~, I2] = sort(D2(end-r+1:end));
% V1_sort = V1(:,end-r+1:end);
% V1_sort = V1_sort(:, I1);
% V2_sort = V2(:,end-r+1:end);
% V2_sort = V2_sort(:, I2);
% figure;imagesc(abs(V2_sort - V1_sort))


% lambda_vec = linspace(1.25, 5, 10);
% all_D = cell(size(lambda_vec));
% 
% for i = 1:length(lambda_vec)
%     lambda = lambda_vec(i);
%     cvx_begin
%         variable D(n_high,n_high)
% %         minimize( norm(D*X1 - X2, 2) + lambda*abs(trace(D)) )
%         minimize( norm(D*X1 - X2, 2) + norm_nuc(D) )
%     cvx_end
%     all_D{i} = D;
% end
% 
% figure;
% plot(lambda_vec, cellfun(@rank, all_D, 'UniformOutput', false))
...
% Get data reconstructions
dat_naive = zeros(size(dat_high));
dat_naive(:,1) = dat_high(:,1);
dat_cvx = dat_naive;
dat_thresh = dat_naive;
dat_exp_matrix = dat_naive;
...
% subplot(6,1,2)
% imagesc(real(dat_naive))
% title('Reconstruction: Naive DMD')
% subplot(6,1,3)
% imagesc(real(dat_thresh))
% title('Reconstruction: PCA-reduced DMD')
% subplot(6,1,4)
% imagesc(real(dat_cvx))
% title('Reconstruction: Nuclear-norm reduced DMD')
% subplot(6,1,5)
