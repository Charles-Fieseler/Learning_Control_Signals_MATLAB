classdef CElegansModelTest_generated_P < matlab.unittest.TestCase
    ...
    methods (Test)
        function testMetadata(testCase)
            ...
            testCase.verifyEqual(...
                mdat{'ID_binary',:}{:}, 1:2);
            testCase.verifyEqual(...
                mdat{'user_custom_control_signal',:}{:}, 3);
        end
        ...
        function testDat(testCase)
            ...
            testCase.verifyTrue(isequal(...
                testCase.model.original_sz, [1,3000]));
            
            testCase.verifyTrue(isequal(...
                testCase.model.total_sz, [4,3000]));
            
            testCase.verifyTrue(isequal(...
                size(testCase.model.control_signal), [3,3000]));
        end
        ...
        function testDynamics(testCase)
            ...
            testCase.verifyTrue(isempty(find(...
                abs(testCase.model.AdaptiveDmdc_obj.A_separate - ...
                testCase.true_matrix) > tol, 1)));
        end
        
        function testReconstruction(testCase)
            % The online data reconstruction
            [x, ~] = testCase.model.generate_time_series(3000);
            
            tol = 1e-6;
            testCase.verifyTrue(isempty(find(...
                abs(testCase.model.dat(1,:) - x(1,:)) > tol, 1)));
        end
    end
